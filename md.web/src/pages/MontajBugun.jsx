    ...
    <div>
        ...
        <div className="card-body" style={{ padding: '0.75rem' }}>
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
            ...
            <button className="btn btn-secondary" onClick={loadData}>
              <StatusIcon icon="🔄" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Yenile
            </button>
          </div>
        </div>
      </div>
      ...
      {jobs.length === 0 ? (
        <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
          <div style={{ fontSize: '3rem', marginBottom: '1rem', display: 'flex', justifyContent: 'center' }}><StatusIcon icon="📅" sx={{ fontSize: 48 }} /></div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ fontSize: '1.1rem', fontWeight: 600 }}>
                        <StatusIcon icon="🏠" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> {job.customerName}
                      </div>
                      <div style={{ fontSize: '0.85rem', opacity: 0.9 }}>
                        <StatusIcon icon="📍" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> {job.location || 'Konum belirtilmedi'}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      {!allCompleted && !hasBlocked && (
                        ...
                        >
                          <StatusIcon icon="✅" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Tek Seferde Tamamla
                        </button>
                      )}
                      <button
                        className="btn btn-sm"
                      ...
                      >
                        ...
                        <div style={{
                          ...
                          flexShrink: 0,
                            ...
                            {task.status === 'completed' ? <StatusIcon icon="✓" sx={{ fontSize: 18 }} /> : task.stageOrder}
                        </div>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', marginBottom: '0.5rem' }}>
                            ...
                            {task.teamName && (
                              <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                                <StatusIcon icon="👷" sx={{ fontSize: 12, verticalAlign: 'middle', mr: 0.5 }} /> {task.teamName}
                              </span>
                            )}
                          </div>
                              ...
                              {pendingIssues.map((issue) => (
                                <div key={issue.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                  <span style={{ fontSize: '0.85rem', color: 'var(--danger)' }}>
                                    <StatusIcon icon="⚠️" sx={{ fontSize: 14, verticalAlign: 'middle', mr: 0.5 }} /> {issue.item} ({issue.quantity} adet) - {issue.note}
                                  </span>
                                  {issue.replacementOrderId && (
                                    ...
                                    >
                                      <StatusIcon icon="✓" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Çözüldü
                                    </button>
                                  )}
                                </div>
                              ))}
                          ...
                          {task.status !== 'completed' && (
                            <div style={{ fontSize: '0.75rem', color: 'var(--info)', marginBottom: '0.25rem' }}>
                              <StatusIcon icon="📷" sx={{ fontSize: 12, verticalAlign: 'middle', mr: 0.5 }} /> Montaj öncesi + sonrası fotoğraf zorunlu
                              {isLast && <><br/><StatusIcon icon="✍️" sx={{ fontSize: 12, verticalAlign: 'middle', mr: 0.5 }} /> Son aşama: Müşteri imzası da zorunlu</>}
                            </div>
                          )}
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                          ...
                          {task.status === 'in_progress' && (
                            <>
                              ...
                              <button
                                ...
                                disabled={actionLoading}
                              >
                                <StatusIcon icon="⚠️" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Sorun
                              </button>
                            </>
                          )}
                          ...
                          {task.status === 'blocked' && pendingIssues.length === 0 && (
                            <button
                              ...
                              disabled={actionLoading}
                            >
                              <StatusIcon icon="▶️" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Devam Et
                            </button>
                          )}
                        </div>
                      </div>
      ...
      <Modal
        ...
        title={<><StatusIcon icon="⚠️" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Montaj Sorunu Bildir</>}
        size="medium"
      >
        {selectedTask && (
          <>
            ...
            <div className="form-group">
              <label className="form-label"><StatusIcon icon="📷" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Fotoğraf (Zorunlu)</label>
              <input
                type="file"
                accept="image/*"
      ...
      <Modal
        ...
        title={selectedJob ? <><StatusIcon icon="✅" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Tüm Görevleri Tamamla</> : <><StatusIcon icon="✅" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Görevi Tamamla</>}
        size="medium"
      >
        ...
        <div className="form-group">
          <label className="form-label"><StatusIcon icon="📷" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Montaj Öncesi Fotoğraf (Zorunlu)</label>
          <input
            type="file"
            accept="image/*"
        ...
        <div className="form-group">
          <label className="form-label"><StatusIcon icon="📷" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Montaj Sonrası Fotoğraf (Zorunlu)</label>
          <input
            type="file"
            accept="image/*"
        ...
        <div className="form-group">
          <label className="form-label">
            <StatusIcon icon="✍️" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Müşteri İmzası 
