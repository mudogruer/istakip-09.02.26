    ...
    switch (widgetId) {
        ...
        return (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
            <div className="widget-stat">
              <div className="widget-stat-icon primary"><StatusIcon icon="📋" /></div>
            ...
            <div className="widget-stat">
              <div className="widget-stat-icon success"><StatusIcon icon="📈" /></div>
              <div className="widget-stat-content">
            ...
            <div className="widget-stat">
              <div className="widget-stat-icon info"><StatusIcon icon="👥" /></div>
              <div className="widget-stat-content">
            ...
            <div className="widget-stat">
              <div className="widget-stat-icon warning"><StatusIcon icon="📅" /></div>
              <div className="widget-stat-content">
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span><StatusIcon icon="📞" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Randevu Bekliyor</span>
              <span className="badge badge-warning">{measureCounts.randevu_bekliyor || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span><StatusIcon icon="📅" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Randevu Alindi</span>
              <span className="badge badge-info">{measureCounts.randevu_alindi || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span><StatusIcon icon="📐" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Olcu Alindi</span>
              <span className="badge badge-success">{measureCounts.olcu_alindi || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span><StatusIcon icon="📏" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Teknik Cizim</span>
      ...
      case 'productionStatus':
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span><StatusIcon icon="⏳" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Bekleyen</span>
              <span className="badge badge-secondary">{prodCounts.pending || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span><StatusIcon icon="📦" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Siparis Verildi</span>
              <span className="badge badge-info">{prodCounts.ordered || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span><StatusIcon icon="🏭" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Uretimde</span>
              <span className="badge badge-primary">{prodCounts.in_production || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span><StatusIcon icon="✅" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Teslim Edildi</span>
            ...
            {(prodCounts.overdue || 0) > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: 'var(--color-danger)' }}>
                <span><StatusIcon icon="⚠️" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Geciken</span>
      ...
      case 'assemblyStatus':
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span><StatusIcon icon="📅" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Planlandi</span>
              <span className="badge badge-info">{asmCounts.scheduled || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span><StatusIcon icon="🔧" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Devam Ediyor</span>
              <span className="badge badge-primary">{asmCounts.in_progress || 0}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span><StatusIcon icon="✅" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Tamamlandi</span>
      ...
      case 'stockAlerts':
        ...
        if (criticalItems.length === 0) {
          return <div className="widget-empty"><div className="widget-empty-icon"><StatusIcon icon="✅" /></div><div className="widget-empty-text">Kritik stok yok</div></div>;
        }
        return (
            ...
            {criticalItems.slice(0, 5).map((item, idx) => (
              <Link key={idx} to="/stok/kritik" className="widget-list-item">
                <div className="widget-list-item-left">
                  <div className="widget-list-item-icon" style={{ background: 'var(--color-danger-bg)', color: 'var(--color-danger)' }}><StatusIcon icon="⚠️" /></div>
      ...
      case 'recentActivities':
        ...
        if (activities.length === 0) {
          return <div className="widget-empty"><div className="widget-empty-icon"><StatusIcon icon="📋" /></div><div className="widget-empty-text">Son aktivite yok</div></div>;
        }
        return (
          <div className="widget-list">
            {activities.map((act, idx) => (
              <Link key={idx} to={`/isler/list?job=${act.id}`} className="widget-list-item">
                <div className="widget-list-item-left">
                  <div className="widget-list-item-icon"><StatusIcon icon="📋" /></div>
                  <div className="widget-list-item-content">
    ...
    <div>
      ...
      <div className="dashboard-header">
        ...
        <div className="dashboard-actions">
          ...
          >
            {editMode ? <><StatusIcon icon="✓" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Duzenlemeyi Bitir</> : <><StatusIcon icon="⚙️" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Duzenle</>}
          </button>
          {editMode && (
            <>
              <button type="button" className="btn btn-secondary" onClick={resetLayout}>
                <StatusIcon icon="↺" sx={{ mr: 0.5, verticalAlign: 'middle' }} /> Sifirla
              </button>
